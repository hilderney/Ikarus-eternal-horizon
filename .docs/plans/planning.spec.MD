...
- **Four exclusive control schemes** (`D19`): Keyboard · Mix (keyboard + mouse) · Gamepad · Touch. **One active at a time.** Default keyboard. Player switches on pause (`G11`). Pointer-steer still deferred. Closes Q07. `C02` stays device-blind; `A02` gates sources.
- Combat vocabulary: move, fire, bomb, switch weapon, switch bomb, dash, pause. Dash input is G0; dash spends the shared D03 pool (`C02` L1–L12). A dedicated jet reserve stays `SHIP-08`.
...
| `SHIP-08` | G1 | should | Jet with its own Energy reserve | Boost drains and recharges when idle | §7 (`C02` `D03` seam). **Dash input + shared-pool spend** is `C02` now |
...
- **Scope:** central input state for **four exclusive schemes (`D19`)**: (1) keyboard, (2) mix = keyboard + mouse buttons/wheel, (3) Gamepad API, (4) touch overlay (`G12`). **One scheme active at a time.** Default **keyboard**. Player picks the scheme on the pause overlay (`G11`). Keyboard: `preventDefault` list; `blur` clears; Shift combos. Gamepad: poll `getGamepads()` while scheme is gamepad. Mouse buttons/wheel only in mix — **no mousemove axis**. Touch overlay only while scheme is touch. Escape always opens pause so the picker is reachable. **Haptics:** `rumble(preset)`. Persistent remap stays G3.
- **Class:** `InputState implements InputPort` — `{ update(dt); isDown(code); axis(id); isPressed(action); consumePress(action); rumble(preset); scheme; setScheme(s); connectedPadCount; dispose() }`.
- **Requires:** A01 (gamepad + mouse + touch + haptics numbers). G12 produces TouchSource.
- **Lib:** — (browser **Gamepad API** + Pointer Events; no third-party pad library; no nipplejs)
- **Rules:** **exactly one scheme active**; default keyboard; combos never shadow the base key; rumble no-ops when no actuator; no per-frame allocation. Axis comes only from the active scheme.
- **Maps:** `SHIP-01`, `SHIP-13` (POC2 play slice), `SHIP-04` (`D19`). **Acceptance:** boot is keyboard-only (mouse click does not fire); pause can switch to Mix / Gamepad / Touch; IJKL debug camera still works from the keyboard.
...
#### `SDD-C01` Ship
- **Folder:** `src/gameobjects/ship/` · spec [ship.spec.ts](../specs/ship.spec.ts)
- **Scope:** `class Ship extends THREE.Group`. Modular airplane rig + **ship sheet** for G08: pose, **byte stats 0–255**, statuses from `BALANCE.ship.cooldowns`, loadout, **slots** (Body integrity/energy, Wings agility/deflection ×4, Shield shield/deflection on top, Weapon nose + level, Bombs twin pylons + level, Collector tail + energyGain, Converter cockpit + labFusion). Body hull is the hitbox. `debugSnapshot(): ShipDebugPort`. D03 writes `stats.energy` and adds collector `energyGain` to regen.
- **Requires:** A01, A03.
...
- **Rules:** nothing outside the ship mutates its slots or its inventory; destruction always disposes all children; G08 must bind the sheet, not a second table.
- **Maps:** `SHIP-01`, `SHIP-03`, `SHIP-06`, `RES-05`, `SHIP-10` (slot mods). **Acceptance:** ship moves within bounds; weapon change updates the tip; modules sit on the body; no GPU leaks on dispose; debugger can read the full ship sheet.

#### `SDD-C02` Controller
- **Folder:** `src/gameobjects/controller/`
- **Scope:** `class PlayerController` maps **device-blind** inputs to ship commands — axis-based force motion (`accel`/`decel`/`brake`, `maxSpeed`), tilt/bank (`rotation.z`, `maxDeg`, `riseMs`/`fallMs`), kinematic **dash** (`consumePress('dash')`, L1–L12 cost/distance, optional D03 `EnergyPort`). Lateral dash barrel-rolls 360° on the tilt axis (180° at midpoint); forward/back-only does not roll. Analog dirs come from `InputPort.axis('moveX'|'moveZ')` which already merged WASD / left stick / nipple (`D19`). `class CameraController` stays keyboard (IJKL/UO + Shift) — debug rig, not a pad/touch/mouse camera. Never imports nipplejs, never listens to pointer events. Fire / bomb / switch / pause are **not** consumed here (E07 / G11).
- **Requires:** A01, A02, A03, B01, C01.
- **Lib:** — (no dependency — input→force mapping; Three.js vectors for integration)
- **Rules:** ship control is continuous force; tilt ramps in `riseMs`, settles in `fallMs`; **effective `maxSpeed`/`accel` read the degradation multiplier from `C03`** — the controller never knows why it is slower. Dash is a bounded impulse; re-dash requires energy and `!status.dashing` (`ship.cooldowns.dashingMs`). Dedicated jet reserve is `SHIP-08`.
- **Maps:** `SHIP-01`, `SHIP-13` (POC2 play), `SHIP-04` (`D19`), `SHIP-08` (dash spend on shared pool). **Acceptance:** the tuned feel of POC-1 (inertia + bank) is preserved on keyboard; left stick, WASD+mouse, and the nipple produce the same force curve after deadzone.
...
- **Rules:** damage enters **only** through `applyDamage` (never by writing fields); all thresholds and multipliers come from `BALANCE.ship.health` until C03 consumes C01 `ship.stats` byte pools; `destroyed` is reported, never acted on here (`G10` ends the run). Do not retune health numbers this pass.
...
- **Folder:** `src/gameobjects/shot/weapon-shot.ts` · spec [weapon-shot.spec.ts](../specs/weapon-shot.spec.ts)
- **Scope:** `class WeaponShot extends THREE.Mesh` — pooled player projectile. `activate(spawn)`, `update(dt)` (logic only), `syncRender()`, `deactivate()`, `dispose()`. Layer **`PlayerShot`** (D13). Constructor never `scene.add` (D14).
- **Requires:** A01, A03, A05.
- **Lib:** **Three.js** (`Mesh`, `BoxGeometry(1, 1, 0.7)`, `MeshBasicMaterial` + additive blending)
- **Contract:** `ShotSpawn { x, z, vx, vz, damage, lifetime, totalLifetime, radius, aoeRadius, range, decayPerUnit, color? }`; `WeaponShotPort { active, layer, x, z, radius, aoeRadius, effectiveDamage() }`. Kinematics arrive on spawn (D02 writes L1: speed 30, lifetime 1, radius 0.12). D01 does **not** read a nested `catalog.projectile`.
- **Decay:** A03 `decayFactor(elapsed)` with `elapsed = 1 − lifetime/totalLifetime`. Same factor drives opacity and `effectiveDamage`. **aoeRadius 0** for laser (D02 writes it; D04 writes >0). **Radius visual:** `scale.x/y = 2 × radius` on unit XY geometry (no hidden `SHOT_BASE_WIDTH`).
- **Rules:** pooled; E04 releases on expiry/hit; `update` never writes GPU; `deactivate` does not dispose GPU.
...
- **Folder:** `src/gameobjects/weapon/` · spec [weapon.spec.ts](../specs/weapon.spec.ts)
- **Scope:** `interface WeaponBehaviour { update(ctx): void; dispose(): void }`; `class Weapon` (device: config + behaviour via the registry seam, `D12` — **does not own the shot pool**); `catalog.ts` (`WEAPONS`; **imported by A01** so `BALANCE.weapons.catalog === WEAPONS`); `registry.ts` (`registerWeapon`); `laser-levels.ts`. **Laser** fills D01 `ShotSpawn` via E04 `ShotAcquirePort`. No `Scene` constructor arg (D14).
- **Requires:** A01, A03, A05, D01, D03, **E04**.
- **Lib:** **Three.js** (bolt visuals via `weapon-shot` only)
- **Rules:** energy gate via `EnergyPort`; rate = `1/(rate × rateMul)`; **adding a weapon = 1 catalog entry + 1 registry entry**. This pass registers `laser` (D04 registers `plasma`). `beam` / `mjolnir` stay catalog-only (G2).
- **Levels:** L1=1f … L12=4f+4/side; **speed 30 · lifetime 1 (range 30)**; play starts at L1. `energyPerShot` is one spend per volley (L1=1 … L12=2.2).
- **Maps:** `WPN-01`, `WPN-04`, `WPN-05`, `SHIP-03`. **Acceptance:** L5 fires 5 bolts; L12 fires 12; registering a new id needs no edit to `Weapon`.

#### `SDD-D03` EnergyManager
- **Folder:** `src/systems/energy-manager.ts`
- **Scope:** `class EnergyManager implements EnergyPort` — shared pool `{ current, max, regenPerSec }`; `canAfford(cost)`, `spend(cost)`, `update(dt)` (regen). Optional `pool` is C01 `stats.energy` so G08 is combat energy. Regen only while `canRegen()` (run binds `ship.status.recovering`) **and** `ship.cooldowns.recoveringMs` after last spend. All fire (and future jets/craft) draw from it.
- **Requires:** A01.
...
- **Folder:** `src/gameobjects/weapon/behaviours/plasma.ts` · spec [plasma.spec.ts](../specs/plasma.spec.ts)
- **Scope:** `class PlasmaBehaviour implements WeaponBehaviour` — slow orbs from the **E04** WeaponShot pool, `aoeRadius` 2.2. AoE is applied by F01 later; this pass orbs fly, fade, and expire.
- **Requires:** D01, D02, D03, E04. (F01 for splash — not a blocker for spawn/switch.)
- **Lib:** **Three.js** (orb mesh via `weapon-shot` with `aoeRadius > 0`)
- **Rules:** the orb is a pooled `WeaponShot`, not a new entity type; `registerWeapon('plasma')` only (D12). Loadout all four weapons; `applyWeaponLevel` scales PLASMA_LEVELS L1–12.
- **Maps:** `WPN-03`, `WPN-04`, `WPN-05`. **Acceptance:** F/LB switches without stalling; orbs are orange and slower than laser. Splash [manual] after F01.
...
#### `SDD-E01` Enemy (basic)
- **Folder:** `src/gameobjects/enemy/` · spec [enemy.spec.ts](../specs/enemy.spec.ts)
- **Scope:** `class Enemy extends THREE.Mesh` — pooled; `active`, `hp`, `takeDamage(amount)`, thin seek toward ship, despawn off-field. Implements `TargetHit` port (`team 'enemy'`, `radius`). Archetypes Tank/Warrior/Rogue → §7. **Yuka SeekBehavior deferred** (first-party drift for now).
- **Requires:** A01, A03, A05. (F01 for shot hits — not a blocker for spawn/visual.)
- **Lib:** **Three.js** (wireframe box mesh)
- **Maps:** `ENM-01`, `ENM-02`. **Acceptance:** spawns from SpawnArea, closes in, despawns off-screen; dies when `takeDamage` empties hp (F01 will call this).
...
#### `SDD-E04` ShotManager
- **Folder:** `src/systems/shot-manager.ts` · spec [shot-manager.spec.ts](../specs/shot-manager.spec.ts)
- **Scope:** **owns** all projectile pools by origin. Weapon / Laser / Plasma only `acquire()`. This pass: real `WeaponShot` pool (capacity 128); enemy/bomb capacity 0 until E03 / WPN-06. Expiry + off-field release; `scene.add` on fill (D14). Feeds `CollisionManager` later.
- **Requires:** A01, A05, D01. **Not** E03 this pass.
- **Lib:** — (pool orchestration; D01 is the mesh)
- **Maps:** `WPN-01`, `RUL-02`. **Acceptance:** shots recycle; exhaustion returns null; no cross-cleanup.

#### `SDD-E05` EnemyManager
- **Folder:** `src/systems/enemy-manager.ts` · spec [enemy-manager.spec.ts](../specs/enemy-manager.spec.ts)
- **Scope:** spawn schedule + live list + pooling + despawn. Spawns inside **SpawnArea** (`BALANCE.enemy.spawn` / live debugger binds). Test stream: `maxActive: 1` every `intervalSec`. F03 rate and Yuka EntityManager deferred.
- **Requires:** A01, A03, A05, E01. (F03 / F01 later.)
- **Lib:** **Three.js** (scene add/remove of Enemy meshes)
- **Maps:** `ENM-01`, `ENM-02`, `RUL-02`. **Acceptance:** one test hostile appears on cadence inside the red volume; population is pooled.
...
- **Requires:** A01, A02, C01, C03, D02, D03. F01 targets list is empty until F01 (beam/cone G2).
...
- **Folder:** `src/ui/debugger/` (one module per tab) · spec [debugger.spec.ts](../specs/debugger.spec.ts)
- **Scope:** **one tab per subject.** This pass: **Ship** (pose, 0–255 pools, statuses) and **Equips** (loadout lists + equipped weapon level 1–12 via E07 + dash level 1–12 via C02). Later: Cam / LimitBox / Parallax / Weapons catalog / Energy / Shots / Collision. Two-way sync ~15 Hz, skip focused control, Reset. Q09 DEV flag.
- **Requires:** A01, C01, G06.
- **Lib:** **HTML/CSS** (panel + controls markup)
- **Rules:** a new tab must not touch existing ones except when a subject moves (loadout → Equips). Tabs must not invent numbers the sim does not own.
- **Maps:** dev tool (no locked requirement; Q09 decides shipping). **Acceptance:** Ship shows the flying craft’s sheet live; Equips weapon level changes the live laser volley; Equips dash level changes dash cost/distance; Reset restores spawn defaults.
...
- **Scope:** `class PauseScene` — Esc **or Start** (`consumePress('pause')`) opens a full-screen HTML/CSS overlay **on top of the Run**, gating `GameLoop.step` through `A04` while keeping the canvas mounted. **Control scheme picker** (Keyboard / Mix / Gamepad / Touch, one active; default keyboard). Inventory / Restart / Quit follow in the rest of G11.
...
- **Jet / dash** — dedicated Energy reserve on the `C02`/`D03` seam — `SHIP-08`. Dash already spends the shared D03 pool.
...
| **D18** | **Gamepad API + dual-rumble haptics in POC2.** `InputState` polls `navigator.getGamepads()` (W3C standard mapping) and plays `GamepadHapticActuator.playEffect('dual-rumble')` while the **gamepad** scheme is active. Persistent remap / Steam Input stay G3 (`SHIP-13`). | `A02`, `C02`, `E07`, `F05`, `G11`, A01 BALANCE |
| **D19** | **Four exclusive control schemes (one at a time).** (1) Keyboard · (2) Mix = keyboard + mouse buttons/wheel — cursor does **not** steer · (3) Gamepad API (`D18`) · (4) touch overlay + nipplejs (`G12`). Default **keyboard**. Switch on **pause** (`G11`). `A02` gates sources; `C02`/`E07` stay device-blind. Escape always pauses so the picker is reachable. Pointer-steer stays deferred (`SHIP-04` remainder). Closes Q07. | `A01`, `A02`, `C02`, `E07`, `G11`, `G12` |
...
| Target build | POC2 (`poc2/`) — Fase 0 playable (A–C, G06/G09/G03/G12); Stage D or G01 next |
...
**Next steps:** 1) play-test Fase 0 (`npm run dev` in `poc2/`); 2) Stage D combat primitives or G01 Title→Run flow.
